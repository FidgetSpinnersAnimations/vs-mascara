...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'bg': //Week uhhhh hmm uhh wait let me think uhhhh
				var bg:BGSprite = new BGSprite('mascara/bg', -600, -300, 0.9, 0.9);
				bg.scale.set(0.8, 0.8);
				add(bg);
			case 'night-bg':
				var bg:BGSprite = new BGSprite('mascara/night_bg', -600, -300, 0.9, 0.9);
				add(bg);
				
				var rain:BGSprite = new BGSprite('mascara/rain', -600, -300, 0.9, 0.9, ['rain'], true);
				rain.setGraphicSize(Std.int(rain.width * 8));
	...
	private function generateSong(dataPath:String):Void
	{
		...
		openfl.Lib.application.window.title = "Friday Night Funkin': V.S. Mascara - " + SONG.song;
	...
	function openChartEditor()
	{
		...
		openfl.Lib.application.window.title = "Friday Night Funkin': V.S. Mascara";
		chartingMode = true;
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					MusicBeatState.switchState(new DemoState());
					openfl.Lib.application.window.title = "Friday Night Funkin': V.S. Mascara";
		...
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					...
					case 'Attack Note': //Hurt note
						FlxG.camera.flash(FlxColor.WHITE, 0.3);
						FlxG.sound.play(Paths.sound('attack_note'));
				}
